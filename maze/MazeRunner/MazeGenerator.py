...
class MazeGenerator:
	...
	def make_maze(self, w, h):
	    ...
	    def walk(x, y):
	        ...
	        d = [(x - 1, y), (x, y + 1), (x + 1, y), (x, y - 1)] # getting possible neighbours
	        shuffle(d) # shuffling possible neighbours
	        for (xx, yy) in d:
	            if grid[yy][xx]: continue # ignore if visited cell or a wall
	            if xx == x: row[max(y, yy)][x] = "X " # randomly
	            if yy == y: col[y][max(x, xx)] = "  " # open walls
